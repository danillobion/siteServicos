...
function adicionarRegistros(dados) {
  ...
  dados.map(e => {
    ...
    cell3.innerHTML = traduzirSentido(e.sentido);
    cell4.innerHTML = tipoDropdown(e);
  });
}
function traduzirSentido(valor){
  if(valor == 0){
    return "Ida"
  }else if(valor == 1){
    return "Volta"
...
function selecionarParada(dados) {
  ...
  let parada_id = dados.getAttribute("parada_id")+"_"+timestamp;
  ...
  cell3.innerHTML = sentidoSelect(parada_id);
  cell4.innerHTML = botaoDeAcoesRemover(parada_id, parada_rua, parada_numero);
}
function sentidoSelect(parada_id) {
  return `
        <select class="form-control" id="sentido_${parada_id}" name="sentidoParada">
          <option value="0">Ida</option>
          <option value="1">Volta</option>
          <option value="2">Inicio</option>
          <option value="3">Fim</option>
          <option value="4">Parada</option>
          <option value="5">Virada</option>
        </select>
        `;
}
